...
_tm_preexec() {
  ...
  (curl -s --max-time 1 "${_TM_URL}/status?pid=$$&state=busy&type=${cmd_type}&title=${PWD##*/}" > /dev/null 2>&1 &) 2>/dev/null
}

_tm_precmd() {
  (curl -s --max-time 1 "${_TM_URL}/status?pid=$$&state=idle&title=${PWD##*/}" > /dev/null 2>&1 &) 2>/dev/null
}
